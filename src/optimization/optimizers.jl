...
function chemical_shift_opt(
    ...
    return @match method begin
        ...
        :SAMIN                   => solve(prob, SAMIN(), f_calls_limit = max_func_evals)
        :ParticleSwarm           => solve(prob, ParticleSwarm(), f_calls_limit = max_func_evals)
        :GN_DIRECT               => solve(prob, Opt(:GN_DIRECT, 7 * sites - 1), maxeval = max_func_evals)
        :GN_DIRECT_L             => solve(prob, Opt(:GN_DIRECT_L, 7 * sites - 1), maxeval = max_func_evals)
        :GN_DIRECT_L_RAND        => solve(prob, Opt(:GN_DIRECT_L_RAND, 7 * sites - 1), maxeval = max_func_evals)
        :GN_DIRECT_NOSCAL        => solve(prob, Opt(:GN_DIRECT_NOSCAL, 7 * sites - 1), maxeval = max_func_evals)
        :GN_DIRECT_L_NOSCAL      => solve(prob, Opt(:GN_DIRECT_L_NOSCAL, 7 * sites - 1), maxeval = max_func_evals)
        :GN_DIRECT_L_RAND_NOSCAL => solve(prob, Opt(:GN_DIRECT_L_RAND_NOSCAL, 7 * sites - 1), maxeval = max_func_evals)
        :GN_DIRECT_ORIG_DIRECT   => solve(prob, Opt(:GN_DIRECT_ORIG_DIRECT, 7 * sites - 1), maxeval = max_func_evals)
        :GN_DIRECT_ORIG_DIRECT_L => solve(prob, Opt(:GN_DIRECT_ORIG_DIRECT_L, 7 * sites - 1), maxeval = max_func_evals)
        :GD_STOGO                => solve(prob, Opt(:GD_STOGO, 7 * sites - 1), maxeval = max_func_evals)
        :GD_STOGO_RAND           => solve(prob, Opt(:GD_STOGO_RAND, 7 * sites - 1), maxeval = max_func_evals)
        :GN_AGS                  => solve(prob, Opt(:GN_AGS, 7 * sites - 1), maxeval = max_func_evals)
        :GN_ISRES                => solve(prob, Opt(:GN_ISRES, 7 * sites - 1), maxeval = max_func_evals)
        :GN_ESCH                 => solve(prob, Opt(:GN_ESCH, 7 * sites - 1), maxeval = max_func_evals)
        :LN_COBYLA               => solve(prob, Opt(:LN_COBYLA, 7 * sites - 1), maxeval = max_func_evals)
        :LN_BOBYQA               => solve(prob, Opt(:LN_BOBYQA, 7 * sites - 1), maxeval = max_func_evals)
        :LN_NEWUOA               => solve(prob, Opt(:LN_NEWUOA, 7 * sites - 1), maxeval = max_func_evals)
        :LN_NEWUOA_BOUND         => solve(prob, Opt(:LN_NEWUOA_BOUND, 7 * sites - 1), maxeval = max_func_evals)
        :LN_PRAXIS               => solve(prob, Opt(:LN_PRAXIS, 7 * sites - 1), maxeval = max_func_evals)
        :LN_SBPLX                => solve(prob, Opt(:LN_SBPLX, 7 * sites - 1), maxeval = max_func_evals)
        :CMAES                   => solve(prob, CMAES(), iterations = max_func_evals ÷ 15)
        :GA                      => solve(prob, GA(), iterations = max_func_evals ÷ 50)
        :ES                      => solve(prob, ES(), iterations = max_func_evals)
        :SeparableNES            => solve(prob, BBO(:separable_nes), MaxFuncEvals = max_func_evals)
