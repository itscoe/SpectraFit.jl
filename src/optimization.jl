...
function ols_cdf(
    ...
    for i = 1:length(parameters)
        powder_pattern[w[i]:w[i+1]] = estimate_powder_pattern(parameters, w[i+1] - w[i] + 1, ν0, I,
...
function quadrupolar_opt_func(x, p)
    if p[6] == 1
        ...
        parameters = map(x -> Quadrupolar(abs(x[5*(i-1)+1]), abs(x[5*(i-1)+2]), abs(x[5*(i-1)+3]), abs(x[5*(i-1)+4])), 1:p[6])
        weights = x[5:5:end]
        if sum(weights) > 1
            weights ./= sum(weights)
        end
